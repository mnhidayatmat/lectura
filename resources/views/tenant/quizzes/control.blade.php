<x-tenant-layout>
    ...
    <div class="space-y-6" x-data="{ }" x-init="setInterval(() => $refs.stateFrame && $refs.stateFrame.click(), 3000)">
        ...
        <div class="grid grid-cols-3 gap-4">
            ...
            <div class="bg-white rounded-2xl border border-slate-200 p-5 text-center">
                <p class="text-2xl font-bold text-slate-900">{{ $currentIndex !== false && $currentIndex >= 0 ? ($currentIndex + 1) : 0 }} / {{ $questions->count() }}</p>
                <p class="text-xs text-slate-500">Question</p>
            </div>
            <div class="bg-white rounded-2xl border border-slate-200 p-5 text-center">
                <p class="text-2xl font-bold text-emerald-600">{{ $activeQ ? $activeQ->responses->count() : '--' }}</p>
        ...
        @elseif($activeQ)
            ...
            <div class="bg-white rounded-2xl border border-slate-200 overflow-hidden">
                <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
                    <span class="text-sm font-medium text-slate-500">Question {{ $currentIndex + 1 }} of {{ $questions->count() }}</span>
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <div class="flex-1">
                                    <div class="w-full bg-slate-100 rounded-lg h-10 relative overflow-hidden">
                                        <div class="h-full rounded-lg transition-all duration-500 {{ $opt->is_correct ? 'bg-emerald-500' : 'bg-indigo-500' }}" style="width: {{ $pct }}%"></div>
