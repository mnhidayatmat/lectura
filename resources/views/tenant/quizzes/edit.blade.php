<x-tenant-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="text-2xl font-bold text-slate-900 dark:text-white">Edit Quiz</h2>
                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Modify questions and settings</p>
            </div>
        </div>
    </x-slot>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-6 space-y-5">
                    ...
                    <label class="flex-1 cursor-pointer">
                        ...
                        <div class="flex items-center gap-3 p-4 rounded-xl border-2 transition peer-checked:border-indigo-500 peer-checked:bg-indigo-50 dark:peer-checked:bg-indigo-900/20 border-slate-200 dark:border-slate-600 hover:border-slate-300">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-slate-500 dark:text-slate-400">Real-time, lecturer-controlled</p>
                            </div>
                        </div>
                    </label>
                    <label class="flex-1 cursor-pointer">
            ...
            <div class="grid sm:grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="text" name="title" required value="{{ old('title', $session->title) }}" placeholder="e.g. Week 3 Review Quiz" class="w-full px-4 py-2.5 rounded-xl border border-slate-300 dark:border-slate-600 dark:bg-slate-700 dark:text-white text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" />
                </div>
                <div>
            ...
            <div>
                ...
                <select name="quiz_folder_id" class="w-full px-4 py-2.5 rounded-xl border border-slate-300 dark:border-slate-600 dark:bg-slate-700 dark:text-white text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                    ...
                    @foreach($folders as $f)
                        <option value="{{ $f->id }}" @selected($f->id == old('quiz_folder_id', $session->quiz_folder_id))>{{ $f->name }}</option>
                    @endforeach
                </select>
            </div>
            @endif
            ...
            <div x-show="category === 'offline'" x-cloak class="grid sm:grid-cols-2 gap-5 pt-2 border-t border-slate-100 dark:border-slate-700">
                <div>
                    ...
                    <input type="datetime-local" name="available_from" :required="category === 'offline'" value="{{ old('available_from', $session->available_from?->format('Y-m-d\TH:i')) }}" class="w-full px-4 py-2.5 rounded-xl border border-slate-300 dark:border-slate-600 dark:bg-slate-700 dark:text-white text-sm focus:ring-2 focus:ring-teal-500 focus:border-teal-500" />
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">Available Until *</label>
                    <input type="datetime-local" name="available_until" :required="category === 'offline'" value="{{ old('available_until', $session->available_until?->format('Y-m-d\TH:i')) }}" class="w-full px-4 py-2.5 rounded-xl border border-slate-300 dark:border-slate-600 dark:bg-slate-700 dark:text-white text-sm focus:ring-2 focus:ring-teal-500 focus:border-teal-500" />
                </div>
                <p class="sm:col-span-2 text-xs text-slate-400 dark:text-slate-500">Students can take the quiz at their own pace within this window.</p>
            </div>
        </div>
    ...
    <script>
        function quizBuilder() {
            return {
                category: '{{ old('category', $session->category ?? 'live') }}',
