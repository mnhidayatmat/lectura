    ...
    @if($courses->isNotEmpty())
        @if($plans->isNotEmpty())
            <form method="GET" class="flex items-center justify-end gap-2 mb-4">
                <label for="sort" class="text-xs font-medium text-slate-500 dark:text-slate-400">Sort plans by</label>
        ...
        {{-- Stats bar (only when plans exist) --}}
        @if($plans->isNotEmpty())
            <div class="flex flex-wrap items-center gap-4 text-xs text-slate-400 mb-6">
                ...
                <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 overflow-hidden">
                            ...
                            <div>
                                ...
                                <p class="text-[11px] text-slate-400">
                                    @if($coursePlans->isNotEmpty())
                                        {{ $coursePlans->count() }} {{ Str::plural('plan', $coursePlans->count()) }} &middot; {{ $coursePlans->sum('activities_count') }} activities
                                    @else
                                        No plans yet
                                    @endif
                                </p>
                    ...
                    @if($coursePlans->isNotEmpty())
                        <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-3 p-4">
                            @foreach($coursePlans as $plan)
                                ...
                                <div class="group bg-slate-50 dark:bg-slate-700/50 rounded-xl border border-slate-200 dark:border-slate-600 hover:border-indigo-200 dark:hover:border-indigo-500 hover:shadow-md transition-all overflow-hidden flex flex-col">
                                    <div class="p-4 flex-1">
                                        ...
                                        <h4 class="text-sm font-semibold text-slate-900 dark:text-white mb-1 line-clamp-2">{{ $plan->title }}</h4>
                                        @if($plan->topic)
                                    ...
                                    <div class="px-4 py-3 border-t border-slate-200 dark:border-slate-600 bg-white/50 dark:bg-slate-800/50 flex items-center justify-between">
                    ...
                    @else
                        <div class="px-5 py-8 text-center">
                            <p class="text-sm text-slate-400 dark:text-slate-500 mb-3">No active learning plans for this course yet.</p>
                            <a href="{{ route('tenant.active-learning.create', [$tenant->slug, $course]) }}" class="inline-flex items-center gap-1.5 text-xs font-medium text-indigo-600 hover:text-indigo-700 transition">
                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>
