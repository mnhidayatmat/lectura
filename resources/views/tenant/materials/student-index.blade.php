<x-tenant-layout>
    <x-slot name="header">
        <div>
            ...
            <p class="mt-1 text-sm text-slate-500">Access learning materials from your enrolled courses</p>
        </div>
    </x-slot>
    ...
    @if($courses->isEmpty())
        <div class="bg-white rounded-2xl border border-dashed border-slate-300 p-16 text-center">
            ...
            <p class="text-sm text-slate-500 max-w-md mx-auto">You'll see materials here once you're enrolled in a course.</p>
        </div>
    @else
        <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            @foreach($courses as $course)
                <a href="{{ route('tenant.materials.student-course', [$tenant->slug, $course]) }}" class="group bg-white rounded-2xl border border-slate-200 hover:border-indigo-200 hover:shadow-md p-5 transition-all">
                    <div class="flex items-start gap-4">
                        ...
                        <div class="min-w-0 flex-1">
                            ...
                            <div class="flex items-center gap-3 mt-2">
                                <span class="text-xs text-slate-400">{{ $course->lecturer->name ?? '' }}</span>
                                <span class="text-slate-300">&middot;</span>
                                <span class="text-xs text-slate-400">{{ $course->files_count }} {{ Str::plural('material', $course->files_count) }}</span>
                            </div>
                        </div>
