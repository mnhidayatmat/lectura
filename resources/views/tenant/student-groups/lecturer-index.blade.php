                        ...
                        <div class="divide-y divide-slate-100 dark:divide-slate-700">
                            @foreach($course->studentGroupSets as $set)
                                <a href="{{ route('tenant.student-groups.show', [$tenant->slug, $course, $set]) }}" class="flex items-center justify-between px-6 py-3 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition">
                                    <div class="flex items-center gap-3">
                                        <span class="text-[10px] font-semibold uppercase tracking-wider px-2 py-0.5 rounded-full {{ match($set->type) { 'lecture' => 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400', 'lab' => 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400', 'tutorial' => 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400' } }}">{{ ucfirst($set->type) }}</span>
                                        <div>
