    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-3">
                <a href="{{ route('tenant.attendance.course', [app('current_tenant')->slug, $session->section->course_id]) }}" class="text-slate-400 hover:text-slate-600 transition">
            ...
            @if($session->status === 'ended' && ! $session->isLocked())
                <div class="flex items-center gap-2">
                    <form method="POST" action="{{ route('tenant.attendance.reopen', [app('current_tenant')->slug, $session]) }}">
                        @csrf
                        <button type="submit" class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-amber-700 bg-amber-50 border border-amber-300 rounded-lg hover:bg-amber-100 transition">
