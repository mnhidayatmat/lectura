<x-tenant-layout>
    <x-slot name="header">
        <h2 class="text-2xl font-bold text-slate-900">Scan QR Attendance</h2>
    </x-slot>
    ...
    <div class="max-w-md mx-auto space-y-6" x-data="qrScanner('{{ route('tenant.attendance.checkin', app('current_tenant')->slug) }}', '{{ csrf_token() }}')">
        ...
        <div class="bg-white rounded-2xl border border-slate-200 overflow-hidden">
            <div class="p-6">
                <div class="text-center mb-4">
                    <p class="text-sm text-slate-600">Point your camera at the QR code displayed by your lecturer</p>
                </div>
        ...
        <div x-show="result" x-cloak x-transition class="rounded-2xl border-2 p-6 text-center"
             :class="result?.error ? 'bg-red-50 border-red-200' : 'bg-emerald-50 border-emerald-200'">
            <div class="w-16 h-16 rounded-full mx-auto mb-4 flex items-center justify-center"
                 :class="result?.error ? 'bg-red-100' : 'bg-emerald-100'">
            ...
            <p class="text-lg font-bold" :class="result?.error ? 'text-red-900' : 'text-emerald-900'" x-text="result?.message || result?.error"></p>
            <p x-show="result?.status" class="mt-1 text-sm" :class="result?.error ? 'text-red-600' : 'text-emerald-600'" x-text="result?.status ? 'Status: ' + result.status : ''"></p>
            <p x-show="result?.checked_in_at" class="mt-1 text-xs text-slate-500" x-text="result?.checked_in_at ? 'Time: ' + result.checked_in_at : ''"></p>

            <button @click="result = null; startCamera()" class="mt-4 px-4 py-2 text-sm font-medium rounded-xl transition"
                    :class="result?.error ? 'bg-red-100 text-red-700 hover:bg-red-200' : 'bg-emerald-100 text-emerald-700 hover:bg-emerald-200'">
                Scan Again
            </button>
        </div>
    </div>
        ...
        function qrScanner(checkinUrl, csrfToken) {
            return {
                ...
                async startCamera() {
                    ...
                    this.scanner = new Html5Qrcode("qr-video", { formatsToSupport: [Html5QrcodeSupportedFormats.QR_CODE] });
                    ...
                    try {
                        ...
                        this.cameraActive = false;
                        this.result = { error: 'Camera access denied. Please allow camera permissions.' };
                    }
                },
