<x-tenant-layout>
    ...
    <div class="grid lg:grid-cols-3 gap-6"
         x-data="{
             requiresSubmission: {{ old('requires_submission', $assessment->requires_submission) ? 'true' : 'false' }},
             requiresGroupSubmission: {{ old('requires_group_submission', $assessment->student_group_set_id ? true : false) ? 'true' : 'false' }},
             selectedGroupSetId: '{{ old('student_group_set_id', $assessment->student_group_set_id) }}',
             groupSets: [],
         ...
         x-init="if (requiresGroupSubmission) fetchGroupSets(); $watch('requiresGroupSubmission', v => { if (v) fetchGroupSets(); })">
                                    ...
                                    <div class="flex items-center gap-3">
                                        <div :class="requiresGroupSubmission ? 'bg-indigo-100 dark:bg-indigo-900/40' : 'bg-slate-100 dark:bg-slate-700/60'"
                                            ...
                                            <svg :class="requiresGroupSubmission ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-400'"
                                                 class="w-4 h-4 transition" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    <div class="px-4 pb-4 pt-3 border-t border-slate-100 dark:border-slate-700 space-y-3">
                                        ...
                                        <template x-if="!loadingGroupSets && groupSets.length === 0">
                                            <div class="rounded-lg border border-amber-200 bg-amber-50 dark:bg-amber-900/15 dark:border-amber-800 p-3 text-xs text-amber-800 dark:text-amber-300">
                                                ...
                                                <p class="mt-1">Create groups first at <a href="{{ route('tenant.student-groups.index', [$tenant->slug, $course]) }}" class="underline font-semibold">Courses → Groups</a>.</p>
                                            </div>
                                        </template>
                                        ...
                                        <div class="flex items-start gap-2 px-3 py-2.5 bg-indigo-50 dark:bg-indigo-900/15 border border-indigo-200 dark:border-indigo-800 rounded-lg">
                                            ...
                                            <p class="text-[11px] text-indigo-700 dark:text-indigo-300">
                                                Only the group leader can submit. Leader is elected via the in-group voting system.
                                            </p>
                                        </div>
                                    </div>
                                </div>
