    ...
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div class="flex items-center gap-3">
                <a href="{{ route('tenant.courses.index', $tenant->slug) }}" class="w-9 h-9 rounded-lg bg-slate-100 hover:bg-slate-200 flex items-center justify-center transition">
    ...
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-3">
            <div class="bg-white rounded-2xl border border-slate-200 p-4">
                ...
                <p class="text-xl font-bold text-slate-900 mt-1">{{ $course->credit_hours ?? '--' }}</p>
            </div>
            <div class="bg-white rounded-2xl border border-slate-200 p-4">
                <p class="text-[10px] font-semibold text-slate-400 uppercase tracking-wider">Weeks</p>
                <p class="text-xl font-bold text-slate-900 mt-1">{{ $course->num_weeks }}</p>
            </div>
            <div class="bg-white rounded-2xl border border-slate-200 p-4">
                <p class="text-[10px] font-semibold text-slate-400 uppercase tracking-wider">Students</p>
                <p class="text-xl font-bold text-indigo-700 mt-1">{{ $course->totalStudents() }}</p>
            </div>
            <div class="bg-white rounded-2xl border border-slate-200 p-4">
                <p class="text-[10px] font-semibold text-slate-400 uppercase tracking-wider">Sections</p>
                <p class="text-xl font-bold text-slate-900 mt-1">{{ $course->sections->count() }}</p>
            </div>
            <div class="bg-white rounded-2xl border border-slate-200 p-4">
                <p class="text-[10px] font-semibold text-slate-400 uppercase tracking-wider">Topics</p>
                <p class="text-xl font-bold text-slate-900 mt-1">{{ $course->topics->count() }}</p>
            </div>
            <div class="bg-white rounded-2xl border border-slate-200 p-4">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-3">
            <a href="{{ route('tenant.materials.manage', [$tenant->slug, $course]) }}" class="group bg-white rounded-2xl border border-slate-200 hover:border-indigo-200 hover:shadow-sm p-4 flex items-center gap-3 transition-all">
                ...
                <div>
                    ...
                    <p class="text-[11px] text-slate-400">Upload & manage</p>
                </div>
            </a>
            <a href="{{ route('tenant.active-learning.index', [$tenant->slug, $course]) }}" class="group bg-white rounded-2xl border border-slate-200 hover:border-teal-200 hover:shadow-sm p-4 flex items-center gap-3 transition-all">
                ...
                <div>
                    <p class="text-sm font-semibold text-slate-900">Active Learning</p>
                    <p class="text-[11px] text-slate-400">{{ $course->activeLearningPlans->count() }} plans</p>
                </div>
            </a>
            <a href="{{ route('tenant.attendance.index', $tenant->slug) }}" class="group bg-white rounded-2xl border border-slate-200 hover:border-emerald-200 hover:shadow-sm p-4 flex items-center gap-3 transition-all">
                ...
                <div>
                    ...
                    <p class="text-[11px] text-slate-400">QR check-in</p>
                </div>
            </a>
            <a href="{{ route('tenant.quizzes.index', $tenant->slug) }}" class="group bg-white rounded-2xl border border-slate-200 hover:border-amber-200 hover:shadow-sm p-4 flex items-center gap-3 transition-all">
                ...
                <div>
                    <p class="text-sm font-semibold text-slate-900">Quizzes</p>
                    <p class="text-[11px] text-slate-400">Live quizzes</p>
                </div>
            </a>
            <a href="{{ route('tenant.student-groups.index', [$tenant->slug, $course]) }}" class="group bg-white rounded-2xl border border-slate-200 hover:border-violet-200 hover:shadow-sm p-4 flex items-center gap-3 transition-all">
            ...
            <a href="{{ route('tenant.whiteboards.index', [$tenant->slug, $course]) }}" class="group bg-white rounded-2xl border border-slate-200 hover:border-fuchsia-200 hover:shadow-sm p-4 flex items-center gap-3 transition-all">
                ...
                <div>
                    <p class="text-sm font-semibold text-slate-900">Whiteboards</p>
                    <p class="text-[11px] text-slate-400">Collaborative</p>
                </div>
            </a>
        </div>
        ...
        @if($course->description || $course->teaching_mode || $course->academicTerm)
            <div class="bg-white rounded-2xl border border-slate-200 p-5">
                <div class="flex items-center gap-6 flex-wrap text-sm text-slate-600">
                    @if($course->teaching_mode)
                        <span class="inline-flex items-center gap-1.5">
                            ...
                            {{ str_replace('_', ' ', ucfirst($course->teaching_mode)) }}
                        </span>
                    @endif
                    ...
                    @if($course->format)
                        <span class="inline-flex items-center gap-1.5">
                            ...
                            {{ collect($course->format)->filter()->keys()->map(fn($f) => ucfirst($f))->implode(', ') }}
                        </span>
                    @endif
                </div>
                @if($course->description)
                    <p class="mt-3 text-sm text-slate-600 leading-relaxed">{{ $course->description }}</p>
                @endif
            </div>
        @endif
