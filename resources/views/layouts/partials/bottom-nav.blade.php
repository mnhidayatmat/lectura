@php
    ...
    $isLive = $active('*/live') || $active('*/session*') || $active('*/quiz/*/play');
@endphp
...
<nav class="fixed bottom-0 inset-x-0 z-40 bg-white/95 dark:bg-[#242d3d]/95 backdrop-blur-md border-t border-slate-200 dark:border-[#354158] safe-area-bottom">
    <div class="flex items-center justify-around h-16 max-w-lg mx-auto px-1">
        ...
        {{-- Home --}}
        <a href="{{ $prefix }}/dashboard" class="flex flex-col items-center justify-center gap-0.5 w-14 py-1 rounded-xl transition {{ $active('*/dashboard') ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-400 active:text-slate-600' }}">
            @if($active('*/dashboard'))
        ...
        {{-- Courses --}}
        <a href="{{ $prefix }}/my-courses" class="flex flex-col items-center justify-center gap-0.5 w-14 py-1 rounded-xl transition {{ $active('*/my-courses*') ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-400 active:text-slate-600' }}">
        ...
        {{-- Scan (FAB center) --}}
        <a href="{{ $prefix }}/scan" class="flex flex-col items-center justify-center gap-0.5 w-14 py-1 rounded-xl transition {{ $active('*/scan') ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-400 active:text-slate-600' }}">
            <div class="w-11 h-11 -mt-5 rounded-2xl {{ $active('*/scan') ? 'bg-indigo-600 shadow-lg shadow-indigo-600/30' : 'bg-slate-900 dark:bg-slate-600 shadow-lg shadow-slate-900/30' }} flex items-center justify-center">
        ...
        {{-- Live --}}
        <a href="{{ $prefix }}/live" class="flex flex-col items-center justify-center gap-0.5 w-14 py-1 rounded-xl transition relative {{ $isLive ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-400 active:text-slate-600' }}">
            @if($isLive)
                <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24"><path d="M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z"/><path fill-rule="evenodd" d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z" clip-rule="evenodd"/></svg>
            @else
        ...
        {{-- Marks --}}
        <a href="{{ $prefix }}/marks" class="flex flex-col items-center justify-center gap-0.5 w-14 py-1 rounded-xl transition {{ $active('*/marks*') ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-400 active:text-slate-600' }}">
            @if($active('*/marks*'))
