    ...
    <div class="flex h-full items-center gap-4">
        ...
        <div class="flex items-center gap-3 min-w-0 flex-shrink-0">
            ...
            <div class="hidden sm:block">
                ...
                <p class="text-[11px] text-teal-500 font-medium leading-tight">Student</p>
            </div>
        </div>

        {{-- Role Switcher --}}
        @php
            $allRoles = auth()->user()->rolesInTenant($tenant->id);
            $userRole = auth()->user()->roleInTenant($tenant->id);
        @endphp
        @if(count($allRoles) > 1)
        <div x-data="{ open: false }" class="relative flex-shrink-0">
            <button @click="open = !open" class="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition border"
                :class="open ? 'bg-teal-50 border-teal-200 text-teal-700 dark:bg-teal-900/30 dark:border-teal-700 dark:text-teal-300' : 'bg-slate-50 border-slate-200 text-slate-600 hover:bg-slate-100 dark:bg-[#2a3548] dark:border-[#354158] dark:text-slate-300 dark:hover:bg-[#354158]'"
            >
                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4"/></svg>
                {{ ucfirst($userRole ?? 'student') }}
            ...
            <div x-show="open" x-cloak @click.away="open = false" x-transition class="absolute left-0 mt-1 w-44 bg-white dark:bg-[#242d3d] rounded-xl shadow-lg border border-slate-200 dark:border-[#354158] z-50 py-1">
                ...
                @foreach($allRoles as $r)
                    @if($r === ($userRole ?? 'student'))
                        <div class="flex items-center gap-2 px-3 py-2 text-sm font-medium text-teal-600 dark:text-teal-400 bg-teal-50 dark:bg-teal-900/20">
                            <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"/></svg>
                            {{ ucfirst($r) }}
                        </div>
                    @else
                        <form method="POST" action="{{ route('tenant.switch-role', $tenant->slug) }}">
                            @csrf
        ...
        {{-- Center: Search bar --}}
        <div class="hidden md:block flex-1 max-w-sm mx-auto">
            <div class="relative">
